...
<?php } else {
    ...
    <p>Ваша учетная запись настроена на <?= Yii::$app->user->can('test_api_user')?"тестовое":"рабочее";?> окружение</p>
    <?php $href = Yii::$app->urlManagerApi->createAbsoluteUrl('/'); ?>
    <p>Документация по API доступна по следующей ссылке: <?= \yii\helpers\Html::a($href, $href, ['target' => '_blank']) ?></p>
